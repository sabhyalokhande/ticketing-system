...
export default async function AllocateSeatsPage({
  ...
  const rowLimit = seatPoolRowLimit(booking.category.name);
  ...
  const seats = allSeats.filter((s) => seatWithinPoolLimit(s.label, booking.category.name));
  ...
  return (
    <main className="mx-auto flex w-full max-w-3xl flex-1 flex-col gap-6 px-4 py-8">
      <header className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-black/60 dark:text-white/60">
            ...
            {seatsFromPool &&
              ` from ${poolName}${rowLimit ? ` (rows up to ${rowLimit})` : ""}`}
          </p>
        </div>
      ...
      {seats.length === 0 ? (
        <p className="text-sm text-black/60 dark:text-white/60">
          No seats exist yet for {poolName}
          {rowLimit ? ` in rows up to ${rowLimit}` : ""}. Add some from the dashboard first.
        </p>
      ) : (
        <SeatPicker
